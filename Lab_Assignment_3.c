...
const int POINTER_SIZE = sizeof(Block);
...
void* my_alloc(int size) {
  ...
  int sizeAfter = (int)(sizeMultiple* POINTER_SIZE);
	...
	while (curr != NULL) {
      ...
      int excess = curr->block_size - sizeAfter - OVERHEAD_SIZE;
...
// };
void menuOptionTwo() {
   int *numOne = (int*)my_alloc(sizeof(int));
   printf("Address of int A: %p\n", numOne); 
   int *numTwo = (int*)my_alloc(sizeof(int));
   printf("Address of int B: %p\n", numTwo); 
   printf("Verifying Results...\n");
   int overheadPlusLarger = OVERHEAD_SIZE + (int)((sizeof(int) > sizeof(void*)) ? sizeof(int) : sizeof(void*));
   printf("Size of overhead + larger of (the size of an integer; the minimum block size): %d bytes\n", overheadPlusLarger); 
   printf("Address B - Address A: %d bytes \n", (int)((long)numTwo - (long)numOne)); 
};
